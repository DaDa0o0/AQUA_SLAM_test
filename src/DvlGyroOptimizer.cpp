                                              ...
                                              double lamda_DVL, double lamda_visual)
{
    ...
    for (size_t i = 0; i < OptKFs.size(); i++) {
        ...
        if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
            ...
            if(v1->estimate().mPoorVision||v2->estimate().mPoorVision){
                ...
                info(1,1) =info(1,1) * 1e2 * lamda_DVL;
                ...
                info(1,1) = info(1,1)*lamda_DVL * 1e3;
    ...
    //     VertexPoseDvlIMU* v2 = dynamic_cast<VertexPoseDvlIMU*>(e->vertices()[1]);
    //     if(v1->estimate().mPoorVision||v2->estimate().mPoorVision){
    //         e->setInformation(Eigen::Matrix<double, 6, 6>::Identity() * lamda_visual * 1);
    //     }
    //     else{
    //         e->setInformation(Eigen::Matrix<double, 6, 6>::Identity() * lamda_visual * 100);
    //     }
    //
    // }
    // for (auto e:acc_edge){
    ...
    //         MapPoint* pMpi = obs.second;
    //         pKFi->EraseMapPointMatch(pMpi);
    //         pMpi->EraseObservation(pKFi);
    //         ROS_INFO_STREAM("remove map point, KF: "<<pKFi->mnId<<", MP: "<<pMpi->mnId<<", chi2: "<<e->chi2());
    //     }
    //     else {
    //         e->setLevel(0);
                                                  ...
                                                  const int &num_fixedKF, double lamda_DVL, double lamda_visual)
{
    ...
    for (size_t i = 0; i < OptKFs.size(); i++) {
        ...
        VP->setFixed(false);
        optimizer.addVertex(VP);
        ROS_INFO_STREAM("FULL BA: opt KF: "<<pKFi->mnId);
    ...
    for (int i = 0; i < FixedKFs.size(); i++) {
        ...
        VP->setFixed(true);
        optimizer.addVertex(VP);
        ROS_INFO_STREAM("FULL BA: fixed KF: "<<pKFi->mnId);
    }
    ...
    for(auto pKFi:OptKFs){
        ...
        VA->setId((maxKFid + 1)*2 + pKFi->mnId);
        VA->setFixed(true);
        // VA->setEstimate(Eigen::Vector3d(0,0,0));
        optimizer.addVertex(VA);
    ...
    for (size_t i = 0; i < OptKFs.size(); i++) {
        ...
        if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
            ...
            epa->setId(optimizer.edges().size());
            epa->setLevel(0);
            ...
            epg->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VG));
            epg->setId(optimizer.edges().size());
            epg->setLevel(0);
    ...
    optimizer.initializeOptimization(0);
    optimizer.save("/home/da/project/ros/orb_dvl2_ws/src/dvl2/data/g2o/full_BA.g2o",0);
    optimizer.setVerbose(true);
    optimizer.optimize(5);
    dvlimu_chi2 = 0;
    visula_chi2 = 0;
