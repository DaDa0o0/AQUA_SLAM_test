...
{
	ros::NodeHandle nh_;
	message_filters::Subscriber<sensor_msgs::Image> image_sub_l(nh_, "suv3d/left/rgb_rect", 2000);
	message_filters::Subscriber<sensor_msgs::Image> image_sub_r(nh_, "suv3d/right/rgb_rect", 2000);
	message_filters::Subscriber<nav_msgs::Odometry> odom_sub(nh_, "odometry/filtered", 2000);
	message_filters::Subscriber<nav_msgs::Odometry> depth_sub(nh_, "/BlueRov2/depth", 2000);
	message_filters::Subscriber<geometry_msgs::PoseStamped> ground_truth(nh_, "/qualisys/BodyROV02/pose", 2000);
	...
	message_filters::Synchronizer<MySyncPolicy>
		synchronizer(MySyncPolicy(10), image_sub_l, image_sub_r, odom_sub, depth_sub, ground_truth);
	synchronizer.registerCallback(boost::bind(&System::frameLoad, this, _1, _2, _3, _4, _5));
	ros::Subscriber dvl_sub = nh_.subscribe("/BlueRov2/DVL", 2000, &System::dvlCallBack, this);

	image_transport::ImageTransport it(nh_);
	image_transport::Publisher img_l_pub = it.advertise("orb_dvl/left_rgb", 10);
	image_transport::Publisher img_r_pub = it.advertise("orb_dvl/right_rgb", 10);
	ros::Publisher gt_pub = nh_.advertise<geometry_msgs::PoseStamped>("orb_dvl/gt", 10);
	mImg_l_pub = &img_l_pub;
	mImg_r_pub = &img_r_pub;
	mGt_pub = &gt_pub;

	ros::spin();
}
void System::frameLoad(const sensor_msgs::ImageConstPtr &img_l,
...
{
	if (!mDepth_initialized || !mOdom_initialized || !mGT_initialized) {
		// initialize depth
		mDepth_initialized = true;
		mInitialized_depth = odom_depth->pose.pose.position.z;

		// initialize ekf
		mOdom_initialized = true;
		Eigen::Isometry3d pose = Eigen::Isometry3d::Identity();
		Eigen::Quaterniond
			r(odom->pose.pose.orientation.w, odom->pose.pose.orientation.x, odom->pose.pose.orientation.y,
			  odom->pose.pose.orientation.z);
		Eigen::Matrix3d r_matrix = r.toRotationMatrix();
		Eigen::Vector3d euler = r_matrix.eulerAngles(0, 1, 2) * 180 / M_PI;
		Eigen::Vector3d t;
		t << odom->pose.pose.position.x, odom->pose.pose.position.y, odom_depth->pose.pose.position.z
			- mInitialized_depth;
		pose.rotate(r.toRotationMatrix());
		pose.pretranslate(t);
		mT_w_e0 = pose;

		// before this mT_e0_ei is T_w_ei

		// mT_e0_ei = mT_w_e0.inverse() * mT_e0_ei;

		// initialize ground truth
		mGT_initialized = true;
		Eigen::Isometry3d T_w_gt = Eigen::Isometry3d::Identity();
		r = Eigen::Quaterniond(gt->pose.orientation.w, gt->pose.orientation.x, gt->pose.orientation.y,
							   gt->pose.orientation.z);

		t << gt->pose.position.x, gt->pose.position.y, gt->pose.position.z;
		T_w_gt.rotate(r);
		T_w_gt.pretranslate(t);
		mT_w_g0 = T_w_gt;
		return;
	}
	...
	try {
		cv_ptr_l = cv_bridge::toCvCopy(img_l, sensor_msgs::image_encodings::BGR8);
		cv_ptr_r = cv_bridge::toCvCopy(img_r, sensor_msgs::image_encodings::BGR8);
	}
	catch (cv_bridge::Exception &e) {
		ROS_ERROR("cv_bridge exception: %s", e.what());
		return;
	}
	double time_rgb = cv_ptr_l->header.stamp.toSec();
	// load EKF pose
	double time_ekf = odom->header.stamp.toSec();
	Eigen::Isometry3d T_w_ej = Eigen::Isometry3d::Identity();
	Eigen::Quaterniond r(odom->pose.pose.orientation.w, odom->pose.pose.orientation.x, odom->pose.pose.orientation.y,
						 odom->pose.pose.orientation.z);
	Eigen::Matrix3d r_matrix = r.toRotationMatrix();
	Eigen::Vector3d euler = r_matrix.eulerAngles(0, 1, 2) * 180 / M_PI;
	Eigen::Vector3d t;
	t << odom->pose.pose.position.x, odom->pose.pose.position.y, odom_depth->pose.pose.position.z - mInitialized_depth;
	T_w_ej.rotate(r.toRotationMatrix());
	// pose.
	T_w_ej.pretranslate(t);

	Eigen::Matrix<double, 6, 1> v_e;
	v_e << odom->twist.twist.linear.x, odom->twist.twist.linear.y, odom->twist.twist.linear.z,
		odom->twist.twist.angular.x, odom->twist.twist.angular.y, odom->twist.twist.angular.z;


	euler = T_w_ej.rotation().eulerAngles(0, 1, 2);
	...
	// load ground truth
	Eigen::Isometry3d T_w_gj = Eigen::Isometry3d::Identity();
	r = Eigen::Quaterniond(gt->pose.orientation.w, gt->pose.orientation.x, gt->pose.orientation.y,
						   gt->pose.orientation.z);
	t << gt->pose.position.x, gt->pose.position.y, gt->pose.position.z;
	T_w_gj.rotate(r);
	T_w_gj.pretranslate(t);
	Eigen::Isometry3d T_g0_gj = mT_w_g0.inverse() * T_w_gj;

	{
		// std::lock_guard<std::mutex> guard(img_lock);
		// Frame f(cv_ptr_l->image, cv_ptr_r->image, time_rgb, mT_w_e0.inverse() * pose, velocity, time_ekf, T_w_gt);
		// Frame f(cv_ptr_l->image, cv_ptr_r->image, time_rgb, pose, velocity, time_ekf,T_w_gt);
		// mvFrames.push_back(f);
		// cout<<"frame num: "<<mvFrames.size()<<endl;


		mImg_l_pub->publish(cv_ptr_l->toImageMsg());
		mImg_r_pub->publish(cv_ptr_r->toImageMsg());


		Eigen::Isometry3d T_e0_ej_gt = mpTracker->mT_g_e.inverse() * T_g0_gj * mpTracker->mT_g_e;
		geometry_msgs::PoseStamped gt_pose;
		gt_pose.header.frame_id = "world";
		gt_pose.header.stamp = gt->header.stamp;
		gt_pose.pose.position.x = T_e0_ej_gt.translation().x();
		gt_pose.pose.position.y = T_e0_ej_gt.translation().y();
		gt_pose.pose.position.z = T_e0_ej_gt.translation().z();
		r = Eigen::Quaterniond(T_e0_ej_gt.rotation());
		gt_pose.pose.orientation.x = r.x();
		gt_pose.pose.orientation.y = r.y();
		gt_pose.pose.orientation.z = r.z();
		gt_pose.pose.orientation.w = r.w();
		mGt_pub->publish(gt_pose);

		Eigen::Isometry3d T_e0_ej = mT_w_e0.inverse() * T_w_ej;
		if (mSensor == System::DVL_STEREO) {
			std::lock_guard<std::mutex> guard(mDVL_state_lock);
			this->TrackStereoDVL(cv_ptr_l->image,
								 cv_ptr_r->image,
								 time_rgb,
								 T_e0_ej,
								 time_ekf,
			...
			this->TrackStereo(cv_ptr_l->image, cv_ptr_r->image, time_rgb);
		}
...
cv::Mat System::TrackStereoDVL(const cv::Mat &imLeft,
							   const cv::Mat &imRight,
							   const double &timestamp,
							   const Eigen::Isometry3d &T_e0_ej,
...
{
	if (mSensor != DVL_STEREO) {
		cerr << "ERROR: you called TrackStereoDVL but input sensor was not set to Stereo-DVL." << endl;
		exit(-1);
	}
	...
	if (mSensor == System::DVL_STEREO) {
		...
		mpTracker->LoadEKFReading(T_e0_ej, timeEKF, good_ekf, T_g0_gj, V_e);
	}
	// std::cout << "start GrabImageStereo" << std::endl;
