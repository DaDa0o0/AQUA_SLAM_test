...
{
public:
	DvlPoint(const float &v_x, const float &v_y, const float &v_z, const double &timestamp): v(v_x, v_y, v_z), t(timestamp){}
...
public:
	...
	Calib(cv::Mat T_gyro_c, cv::Mat T_dvl_c): mT_gyro_c(T_gyro_c), mT_dvl_c(T_dvl_c)
	{
		mT_c_gyro = cv::Mat::eye(4,4,CV_32F);
		mT_c_gyro.rowRange(0,3).colRange(0,3) = mT_gyro_c.rowRange(0,3).colRange(0,3).t();
		mT_c_gyro.rowRange(0,3).col(3) = -mT_gyro_c.rowRange(0,3).colRange(0,3).t()*mT_gyro_c.rowRange(0,3).col(3);
	...
	mT_gyro_c(calib.mT_gyro_c.clone()),mT_dvl_c(calib.mT_dvl_c.clone()),mT_c_gyro(calib.mT_c_gyro.clone()),mT_c_dvl(calib.mT_c_dvl.clone()),
